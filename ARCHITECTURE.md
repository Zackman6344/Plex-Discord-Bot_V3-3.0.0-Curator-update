...
All three files this feature owns are written through a `.tmp` sibling and renamed over the target, so a kill between the truncate and the write cannot leave a half-file. A file that fails to parse at startup puts its store **read-only for the run** and logs an error naming the path, rather than being replaced by whatever the process happens to hold. The old shape (parse-fail, reset the cache to empty, then overwrite on the next write) turned one interrupted write into a permanently lost tally, which is the one thing here that cannot be re-derived from any server. `helpers/tagSidecar.js:118` had the pattern already.
...
**No reply from this command may ping.** Every send goes through one `say()` helper that sets `allowedMentions: { parse: [] }`, and the connect flow's status edits go through `editQuietly()`. Replies quote slot names, player names and server error text, none of which the bot authors: `slotArg` strips user mentions but not role mentions, so `!ap unclaim <@&role>` had the bot echo a live role mention, and the participant role it creates is mentionable. Suppressing at the one helper rather than at the four sites that happened to interpolate user text is what stops the next reply reintroducing it.
...
**Only the connection's own team is counted.** `client.goaled` carries every team the server reports and slot numbers repeat across them, while a claim has no team at all, so `recordGoals` skips any id whose team is not `client.team`. `slotNames` is keyed `team:slot` to match `goaled`/`released`/`fullyChecked`; keyed by slot number alone, team 1's name overwrote team 0's and either team's goal was credited to whoever claimed the surviving name.
