...
- `!ap next [id] [slot]`: The soonest locations you can **actually reach**. With no slot named it answers for **every slot you have claimed**; name one to narrow it. Only ever your own slots. Held to spheres the slot has proven it can enter, so it never points at a door that is still locked, and it never says what is in them. Slots that have already goaled or been released are skipped and counted off at the end, so a mostly-finished async still answers about the slots you are actually playing. Needs the seed's spoiler log at `data/archipelago/spoilers/<seed_name>.txt`; the command names the exact path when it is missing. The room's own sphere tracker cannot substitute, because it only lists locations that have already been checked.
