...
- `!ap next [id] [slot]`: The unchecked locations in the earliest sphere a slot still has anything open in. Ordered by sphere and nothing else, and it never says what is in them. Needs the seed's spoiler log at `data/archipelago/spoilers/<seed_name>.txt`; the command names the exact path when it is missing. The room's own sphere tracker cannot substitute, because it only lists locations that have already been checked.
